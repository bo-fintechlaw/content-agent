...
export function createSlackWebhookRouter(supabase, config) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        if (
          !verifySlackRequestWithSecrets(req, [
            config.SLACK_SIGNING_SECRET,
            config.SLACK_CMO_SIGNING_SECRET,
          ])
        ) {
          return res.status(401).json({ ok: false });
        }
...
function verifySlackRequest(req, signingSecret) {
  return verifySlackRequestWithSecrets(req, [signingSecret]);
}

function verifySlackRequestWithSecrets(req, signingSecrets) {
  const secrets = signingSecrets.filter((secret) => String(secret ?? '').trim());
