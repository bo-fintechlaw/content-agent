 ...
 * its own retry-deferred notification). The user needs to know the draft existed,
 * what the scores were, and why it was rejected so they can manually revise or
 * archive.
 *
 * @param {import('@slack/web-api').WebClient} client
 * @param {string} channel
 ...
 * }} payload
 */
export async function sendDraftRejectedNotification(client, channel, payload) {
  start('sendDraftRejectedNotification');
  const channelId = normalizeChannelId(channel);

  const scoreLine = ['accuracy', 'engagement', 'seo', 'voice', 'structure']
    .map((k) => `${k}: ${Number(payload.scores?.[k] ?? 0).toFixed(1)}`)
    .join('  •  ');
  const flagLine = (payload.flags ?? []).slice(0, 8).join(', ') || '(none)';
  const revisionBlock = (payload.revisionNotes ?? [])
    .slice(0, 5)
    .map((n) => `• ${truncate(String(n), 250)}`)
    .join('\n');
  const manualBlock = (payload.manualVerificationNotes ?? [])
    .slice(0, 5)
    .map((n) => `• ${truncate(String(n), 350)}`)
    .join('\n');

  const blocks = [
    {
      type: 'header',
      text: { type: 'plain_text', text: 'Draft REJECTED by judge', emoji: true },
    },
    { type: 'divider' },
    {
      type: 'section',
      text: {
        type: 'mrkdwn',
        text:
          `*${payload.blogTitle || '(untitled)'}*\n` +
          `*Composite:* ${Number(payload.composite ?? 0).toFixed(1)}  •  ${scoreLine}\n` +
          `*Flags:* ${flagLine}\n` +
          `*Draft ID:* \`${payload.draftId}\`  •  *Topic:* \`${payload.topicId}\``,
      },
    },
  ];
  ...
  if (revisionBlock) {
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `*What to fix:*\n${revisionBlock}` },
    });
  }
  if (payload.reviewUrl) {
    blocks.push({
      ...
      text: {
        type: 'mrkdwn',
        text: `<${payload.reviewUrl}|Open full draft preview>`,
      },
    });
  }
  ...
  const result = await breaker.execute(
    () =>
      client.chat.postMessage({
        channel: channelId,
        text: `Draft rejected: ${payload.blogTitle || payload.draftId}`,
        blocks,
      }),
    { ok: false, error: 'slack_unavailable' }
  );
  if (!result.ok) {
    const err = new Error(String(result.error ?? 'slack_post_failed'));
    fail('sendDraftRejectedNotification', err, { channelId });
  } else {
    success('sendDraftRejectedNotification', { ts: result.ts });
  }
  return result;
}
