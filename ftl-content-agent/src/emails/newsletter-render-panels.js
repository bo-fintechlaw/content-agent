...
function emailHeaderRow(issue, dateStr) {
  return `<tr><td style="padding:28px 28px 16px;background:linear-gradient(135deg,${brand.purple} 0%,${brand.purpleDark} 100%);">
...
function emailIntroRow(issue) {
  return `<tr><td style="padding:28px 28px 8px;">
    <p style="margin:0 0 6px;font:600 11px/1.4 ${fonts.ui};color:${brand.pink};letter-spacing:.08em;text-transform:uppercase;">From ${escapeHtml(issue.author.name)}</p>
...
function emailFooterRow(issue, urls) {
  const links = [];
  if (urls.archiveUrl) {
    links.push(`<a href="${escapeAttr(urls.archiveUrl)}" style="color:${brand.purple};font-weight:600;">View on web</a>`);
  }
  if (urls.unsubscribeUrl) {
    links.push(`<a href="${escapeAttr(urls.unsubscribeUrl)}" style="color:${brand.purple};font-weight:600;">Unsubscribe</a>`);
  }
...
function renderEmailFeature(panel, section, kicker) {
    ...
    .map(
      (s) =>
        `<td align="center" style="padding:8px 12px;background:${brand.surfaceAlt};border:1px solid ${brand.border};">
          <p style="margin:0;font:700 22px/1.2 ${fonts.heading};color:${brand.purple};">${escapeHtml(s.value)}</p>
  ...
  const takeawaysBlock = takeaways
    ? `<tr><td style="padding:0 0 16px;">
        <p style="margin:0 0 8px;font:600 11px/1.4 ${fonts.ui};color:${brand.pink};letter-spacing:.06em;text-transform:uppercase;">Key takeaways</p>
  ...
  const callout = panel.pull_quote
    ? `<tr><td style="padding:12px 16px;background:${brand.surfaceAlt};border-left:3px solid ${brand.pink};">
  ...
  return `<tr><td style="padding:24px 28px;border-top:1px solid ${brand.border};">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="padding-bottom:12px;">
        <p style="margin:0 0 4px;font:600 10px/1.4 ${fonts.ui};color:${brand.purple};letter-spacing:.1em;text-transform:uppercase;">${escapeHtml(section)}</p>
      ...
      <tr><td style="padding-top:16px;">
        <a href="${escapeAttr(panel.blog_url)}" style="display:inline-block;padding:10px 18px;background:${brand.purple};color:#fff;font:600 14px/1 ${fonts.ui};text-decoration:none;border-radius:4px;">Read the full analysis →</a>
      </td></tr>
    </table>
  </td></tr>`;
}
...
function renderEmailCompliance(panel, section, kicker) {
  const deadlines = (panel.deadlines ?? [])
    .map(
      (d) =>
        `<tr>
          <td style="padding:8px 12px 8px 0;font:600 13px/1.4 ${fonts.ui};color:${brand.purple};white-space:nowrap;vertical-align:top;">${escapeHtml(d.date)}</td>
  ...
  const deadlinesBlock = deadlines
    ? `<p style="margin:16px 0 8px;font:600 11px/1.4 ${fonts.ui};color:${brand.pink};letter-spacing:.06em;text-transform:uppercase;">Deadlines</p>
  ...
  const litigationBlock = litigation
    ? `<p style="margin:16px 0 8px;font:600 11px/1.4 ${fonts.ui};color:${brand.pink};letter-spacing:.06em;text-transform:uppercase;">Litigation watch</p>
...
function renderEmailActionItems(panel, section, kicker) {
  const groups = (panel.groups ?? [])
    .map((g) => {
      ...
      return `<p style="margin:16px 0 8px;font:600 13px/1.4 ${fonts.ui};color:${brand.purple};">${label}${escapeHtml(g.firm_type)}</p>
  ...
  return `<tr><td style="padding:24px 28px;border-top:1px solid ${brand.border};">
    ...
    <p style="margin:0 0 8px;font:400 16px/1.55 ${fonts.body};color:${brand.muted};">${escapeHtml(panel.dek)}</p>
    ${groups}
    <p style="margin:16px 0 0;"><a href="${escapeAttr(panel.consultation_url)}" style="color:${brand.purple};font:600 14px/1 ${fonts.ui};">Schedule a consultation →</a></p>
  </td></tr>`;
}
...
function webStyles() {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .masthead { display: flex; align-items: center; gap: 1rem; padding: 1.5rem; margin: -2rem -1.25rem 2rem; background: linear-gradient(135deg, var(--purple) 0%, var(--purple-dark) 100%); color: #fff; }
    ...
    .from-label { margin: 0 0 0.5rem; font-family: ${fonts.ui}; font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; color: var(--pink); font-weight: 600; }
    .intro-text { margin: 0; font-size: 1.1rem; }
    .toc { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border); }
    .section-label { margin: 0 0 0.75rem; font-family: ${fonts.ui}; font-size: 0.7rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); font-weight: 600; }
    ...
    .panel-section { margin: 0 0 0.25rem; font-family: ${fonts.ui}; font-size: 0.65rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--purple); font-weight: 600; }
    ...
    .stat-value { margin: 0; font-family: ${fonts.heading}; font-size: 1.5rem; color: var(--purple); }
    ...
    .takeaways-label, .sub-label { margin: 1rem 0 0.5rem; font-family: ${fonts.ui}; font-size: 0.7rem; letter-spacing: 0.06em; text-transform: uppercase; color: var(--pink); font-weight: 600; }
    .takeaways { margin: 0; padding-left: 1.25rem; }
    .callout { margin: 1rem 0; padding: 1rem 1.25rem; background: var(--surface-alt); border-left: 3px solid var(--pink); }
    ...
    .cta { display: inline-block; margin-top: 1rem; padding: 0.65rem 1.1rem; background: var(--purple); color: #fff; font-family: ${fonts.ui}; font-size: 0.9rem; font-weight: 600; text-decoration: none; border-radius: 4px; }
    ...
    .deadline-date { font-family: ${fonts.ui}; font-weight: 600; color: var(--purple); white-space: nowrap; }
    .action-group { margin-top: 1rem; }
    .action-group-title { margin: 0 0 0.5rem; font-family: ${fonts.ui}; font-weight: 600; color: var(--purple); }
